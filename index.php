  ...
  if (!empty($_GET['$category_id'])) {
	...
	if (empty($_POST['search']) && !isset($_COOKIE['search'])) {
	  	if (!empty($_GET['category_id'])) {
				...
				$pdostatement = $pdo->prepare("SELECT * FROM products WHERE category_id=$category_id ORDER BY id DESC");
				$pdostatement->execute();
				$rawResult = $pdostatement->fetchAll();

				$totalpages = ceil(count($rawResult)/$frames);

				$pdostatement = $pdo->prepare("SELECT * FROM products WHERE category_id=$category_id ORDER BY id DESC LIMIT $offset,$frames");
				$pdostatement->execute();
						...
						<div class="single-product">
							...
							<div class="product-details">
								...
								<div class="price">
									...
									<h6><?php echo $value['quantity'] == 0 ? 'Out of stock' : 'In stock'.' '.$value['quantity']; ?></h6>
								</div>
